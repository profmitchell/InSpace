...
import { SettingsPanel } from "../gizmo/SettingsPanel"
...
interface GizmoPlaygroundProps {
  initialMode?: "transform" | "rotate"
}

export const GizmoPlayground: React.FC<GizmoPlaygroundProps> = ({ initialMode = "transform" }) => {
  const [mode, setMode] = useState<"transform" | "rotate">(initialMode)
  ...
  return (
    <div className="flex flex-col items-center gap-8 p-6 bg-gray-900 rounded-xl">
      <div className="flex items-center justify-between w-full">
        <h1 className="text-xl font-bold text-white">InSpace Gizmo Controller</h1>
        <button
      ...
      <div className="flex flex-wrap items-center justify-center gap-8">
        {/* Demo objects selection */}
        <div className="flex flex-col items-center gap-4 p-4 bg-gray-800/50 backdrop-blur-md rounded-xl">
          <h2 className="text-white font-medium">Demo Objects</h2>
          <div className="flex gap-4">
            <button
              onClick={() => setActiveDemo("cube")}
              className={`p-2 rounded-lg ${activeDemo === "cube" ? "bg-white/30" : "bg-white/10 hover:bg-white/20"}`}
            >
              <div className="w-16 h-16 bg-white/20 border border-white/30 rounded-sm flex items-center justify-center">
                <span className="text-white/70 text-xs">Cube</span>
              </div>
            </button>
            <button
              onClick={() => setActiveDemo("pyramid")}
              className={`p-2 rounded-lg ${activeDemo === "pyramid" ? "bg-white/30" : "bg-white/10 hover:bg-white/20"}`}
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg ${activeDemo === "sphere" ? "bg-white/30" : "bg-white/10 hover:bg-white/20"}`}
            >
        ...
        <div className="relative">
          <div className="w-64 h-64 bg-gray-800/50 backdrop-blur-md rounded-xl shadow-inner flex items-center justify-center perspective-500">
            <DemoObject position={position} rotation={rotation} type={activeDemo} size={64} />
          </div>
          <div className="absolute bottom-2 left-2 text-xs text-white/70">
            {mode === "transform" ? "Position" : "Rotation"}:
            {mode === "transform"
        ...
        <div className="relative">
          ...
          />
          {showSettings && (
            <SettingsPanel settings={settings} updateSettings={updateSettings} onClose={() => setShowSettings(false)} />
          )}
        </div>
      </div>
    </div>
